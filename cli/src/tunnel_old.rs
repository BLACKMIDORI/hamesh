...
//     socket_std: std::net::UdpSocket,
//     source_peer: SocketAddr,
//     dest_peer: SocketAddr,
// ) {
//     let socket = tokio::net::UdpSocket::from_std(socket_std).unwrap();
//     socket
//         .connect(dest_peer)
//         .await
//         .expect("Failed to connect to remote UDP");
//     info!(
//         "connected {} <-> {}",
...
//             "--inbound" => {
//                 input_settings.insert(
//                     format!("{host_port}:{remote_host_port}"),
//                     PortSettings {
//                         protocol,
//                         host_port,
//                         remote_host_port,
...
//     tokio::join!(
//         async {
//             tokio::spawn(datagram_handler(
//                 datagram_handler_sender_dest,
//                 handler_to_writer_sender_to_clone.clone(),
//                 input_clients_sender,
//                 output_settings,
//             ));
//             tokio::spawn(datagram_from_local_client_to_tunnel_wrapper(
//                 socket_std.try_clone().unwrap(),
//                 handler_to_writer_sender_to_clone.clone(),
//             ));
//             tokio::spawn(input_clients_handler(
//                 handler_to_writer_sender_to_clone.clone(),
//                 receiver,
//                 input_settings_clone,
//             ));
//             delivery_syn(socket_std.try_clone().unwrap(), received_syn_ack).await;
//             delivery_input_ports(
//                 socket_std.try_clone().unwrap(),
//                 &input_settings,
//                 &inputs_ack_rx,
//             )
...
//             send_syn(tokio::net::UdpSocket::from_std(socket_std.try_clone().unwrap()).unwrap())
//                 .await;
//             interval.tick().await;
//         } else {
//             break;
...
// }
//
// async fn delivery_input_ports(
//     socket_std: std::net::UdpSocket,
//     input_settings: &HashMap<String, PortSettings>,
//     inputs_ack_rx: &std::sync::mpsc::Receiver<String>,
// ) {
//     let mut interval = time::interval(Duration::from_secs(1));
...
//                 send_input_port(
//                     tokio::net::UdpSocket::from_std(socket_std.try_clone().unwrap()).unwrap(),
//                     id.as_str(),
//                     value,
//                 )
//                 .await;
...
//     datagram_handler_sender: Sender<ControlDatagram>,
//     writer_sender: Sender<ControlDatagram>,
//     input_clients_sender: tokio::sync::mpsc::Sender<ControlDatagram>,
//     output_settings: HashMap<String, PortSettings>,
// ) {
...
//                         };
//                         let output_settings_option =
//                             output_settings.iter().find(|(_, settings)| {
//                                 settings.protocol == input_settings.protocol
//                                     && settings.remote_host_port == input_settings.host_port
//                                     && settings.host_port == input_settings.remote_host_port
//                             });
...
//
//                                             let output_server =
//                                                 OutputServer::Tcp(OutputServerConfigTcp::new(
//                                                     reader_sender_clone,
//                                                     writer_sender_clone,
//                                                     address,
//                                                 ));
...
//                                             }
//                                             let output_server =
//                                                 OutputServer::Udp(OutputServerConfigUdp::new(
//                                                     reader_sender_clone,
//                                                     writer_sender_clone,
//                                                     address,
//                                                 ));
...
//                             Err(error) => {
//                                 error!(
//                                     "could not send the received client data to broadcast: {}",
//                                     error
//                                 )
//                             }
//                         }
//                     }
//                     "client_data" => match input_clients_sender.send(datagram).await {
//                         Ok(_) => {}
//                         Err(error) => {
//                             error!(
//                                 "could not send the received client data to broadcast: {}",
...
// }
//
// async fn input_clients_handler(
//     tunnel_writer_sender: Sender<ControlDatagram>,
//     mut remote_client_data_receiver: tokio::sync::mpsc::Receiver<ControlDatagram>,
//     input_settings: HashMap<String, PortSettings>,
// ) {
...
//                                         "{}_{}_{}",
//                                         protocol_str,
//                                         client_data_settings.host_port,
//                                         client_data_settings.host_client_port
//                                     );
//                                     let mut client_option =
//                                         clients.get(client_id.as_str()).cloned();
//                                     if client_option.is_none() {
//                                         let (to_inside, _) = channel(1024);
//                                         let client =
//                                             InputClient::new(client_data_settings, to_inside);
//                                         clients.insert(client_id.clone(), client.clone());
//                                         info!("client inserted: {}", client_id.as_str());
//                                         client_option = Some(client.clone());
//                                         client
//                                             .start(input_settings, tunnel_writer_sender.clone())
//                                             .await;
//                                     }
...
//                             settings.protocol == client_data_settings.protocol
//                                 && settings.remote_host_port == client_data_settings.host_port
//                         });
//
//                         match input_settings_option {
//                             Some((_, input_settings)) => {
//                                 match sender
//                                     .send(InputClientData {
...
//                                     Err(error) => {
//                                         error!(
//                                             "could not send data to input clients channel: {error}"
//                                         );
//                                     }
//                                 }
//                             }
...
// }
//
// async fn datagram_from_local_client_to_tunnel_wrapper(
//     socket_std: std::net::UdpSocket,
//     writer_sender: Sender<ControlDatagram>,
// ) {
...
//     socket_std: std::net::UdpSocket,
//     received_syn_ack: &AtomicBool,
//     inputs_finished: &AtomicBool,
//     inputs_sender: &std::sync::mpsc::Sender<String>,
//     datagram_handler_sender: &Sender<ControlDatagram>,
// ) {
...
//             Ok(size) => match std::str::from_utf8(&buff[..size]) {
//                 Ok(data) => match serde_json::from_str::<ControlDatagram>(data) {
//                     Ok(maybe_fragment) => {
//                         if maybe_fragment.version != 1 {
//                             error!(
//                                 "🔻 RECEIVED {} with unrecognized version",
//                                 maybe_fragment.r#type
//                             );
...
//                                 let index: i32 =
//                                     maybe_fragment.content["index"].clone().parse().unwrap();
//                                 let digest = maybe_fragment.content["digest"].clone();
//                                 if !fragment_datagrams.iter().any(|fragment_datagram| {
//                                     fragment_datagram.content["digest"] == digest
//                                         && fragment_datagram.content["index"]
//                                             == maybe_fragment.content["index"]
//                                 }) {
//                                     fragment_datagrams.push_front(maybe_fragment.clone());
//                                 }
//                                 let length: i32 =
//                                     maybe_fragment.content["length"].clone().parse().unwrap();
//                                 if index == length - 1 {
//                                     let mut fragments_data_list =
...
//                                             let fragment_index: i32 =
//                                                 fragment_datagram.content["index"].parse().unwrap();
//                                             fragments_data_list[fragment_index as usize] =
//                                                 Some(fragment_datagram.content["data"].clone());
//                                             to_remove.push(i);
//                                         }
//                                     }
...
//                                         }
//                                         let control_datagram_result =
//                                             serde_json::from_str::<ControlDatagram>(
//                                                 restored_data.as_str(),
//                                             );
...
//                                         info!(
//                                             "🔻 RECEIVED {}{{id:{}}}",
//                                             control_datagram.r#type, control_datagram.content["id"]
//                                         );
...
//                                             match inputs_sender
//                                                 .send(control_datagram.content["id"].to_string())
//                                             {
//                                                 Ok(_) => {
...
//                                                 Err(_) => {
//                                                     error!(
//                                                         "could not send ACK for handling: {:?}",
//                                                         control_datagram
//                                                     )
//                                                 }
//                                             }
//                                         } else {
//                                             match datagram_handler_sender
//                                                 .send(control_datagram.clone())
//                                             {
//                                                 Ok(_) => {}
//                                                 Err(_) => {
//                                                     let r#type = control_datagram
//                                                         .r#type
//                                                         .as_str()
//                                                         .to_string();
//                                                     let id = control_datagram
//                                                         .content
//                                                         .get("id")
//                                                         .cloned()
...
//                                         let id_option_str =
//                                             id_option.and_then(|text| Some(text.to_string()));
//                                         match datagram_handler_sender.send(control_datagram) {
//                                             Ok(_) => {}
//                                             Err(_) => match id_option_str {
//                                                 Some(id) => {
//                                                     error!("could not send datagram for handling: {:?}{{id:{}}}", r#type, id);
//                                                 }
//                                                 None => {
...
//     info!(
//         " ⃤ SENT {}{{id:{}}}",
//         datagram.r#type, datagram.content["id"]
//     );
// }
//
...
//                     Err(error) => {
//                         error!(
//                             "Failed to send {} (fragment index 0): {error}",
//                             datagram.r#type
//                         );
//                     }
//                 }
...
//                     Err(error) => {
//                         error!(
//                             "Failed to send {} (fragment index 1): {error}",
//                             datagram.r#type
//                         );
//                     }
//                 }
...
//                 " ⃤ SENT {}{{id:{}}}",
//                 datagram.r#type,
//                 datagram.content.get("id").unwrap_or(&"null".to_string())
//             );
//         }
//         Err(error) => {
