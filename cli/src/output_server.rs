                            ...
                            match result {
                                Ok((tcp_stream, local_client_address)) => {
                                    ...
                                    let reader_handler = tokio::spawn(async move {
                                        ...
                                        loop {
                                            ...
                                            match read_stream.read_buf(&mut buff).await {
                                                ...
                                                Err(error) => {
                                                    error!(
                                                        "could not read local client data: {}",
                                                        error
                                                    );
                                                    break;
                                                }
                                    ...
                                    tokio::spawn(async move {
                                        ...
                                        support_sender
                                            .send(ControlDatagram::server_data(
                                                "close",
                                                0,
                                                ServerDataSettings {
                                                    protocol: Protocol::Tcp,
                                        ...
                                        loop {
                                            match receiver.recv().await {
                                                Ok(datagram) => {
                                                    match datagram.r#type.as_str() {
                                                        "server_data" => {
                                                            let protocol_str = datagram.content
                                                                ["protocol"]
                                                                .as_str();
                                                            let remote_host_port_str = datagram
                                                                .content["remoteHostPort"]
                                                                .as_str();
                                                            let remote_host_client_port_str =
                                                                datagram.content
                                                                    ["remoteHostClientPort"]
                                                                    .as_str();
                                                            let server_data_settings =
                                                                ServerDataSettings {
                                                                    ...
                                                                    remote_host_port:
                                                                        remote_host_port_str
                                                                            .parse()
                                                                            .unwrap(),
                                                                    remote_host_client_port:
                                                                        remote_host_client_port_str
                                                                            .parse()
                                                                            .unwrap(),
                                                                };
                                                            if server_data_settings.protocol
                                                                == Protocol::Tcp
                                                                && server_data_settings
                                                                    .remote_host_port
                                                                    == host_port
                                                                && server_data_settings
                                                                    .remote_host_client_port
                                                                            ...
                                                                            > sequence
                                                                        {
                                                                            ...
                                                                            match id_option {
                                                                                Some(id) => {
                                                                                    ...
                                                                                    match tunnel_sender.send(ControlDatagram::ack_old(id)) {
                                                                                        Ok(_) => {}
                                                                                        Err(error) => {
            ...
            OutputServer::Udp(server) => {
                ...
                match server_socket_result {
                    Ok(server_socket) => {
                        ...
                        let mut receiver = broadcast_sender.subscribe();
                        let (server_socket_sender, mut server_socket_sender_receiver) =
                            channel::<(Vec<u8>, SocketAddr)>(1024);
                        let (server_socket_receiver_sender, mut server_socket_receiver) =
                            channel::<(Vec<u8>, usize, SocketAddr)>(1024);
                        tokio::spawn(async move {
                            tokio::join!(
                                async {
                                    loop {
                                        ...
                                        match server_socket.recv_buf_from(&mut buff).await {
                                            Ok((size, address)) => {
                                                let _ = server_socket_receiver_sender
                                                    .send((buff, size, address))
                                                    .await;
                                            }
                                            Err(_) => {}
                                        }
                                ...
                                async {
                                    loop {
                                        match server_socket_sender_receiver.recv().await {
                                            Some((bytes, address)) => {
                                                let _ = server_socket
                                                    .send_to(bytes.as_slice(), address)
                                                    .await;
                                            }
                                            None => {}
                                        }
                            ...
                            loop {
                                match server_socket_receiver.recv().await {
                                    Some((buff, size, udp_client)) => {
                                        ...
                                        let id = format!(
                                            "udp_client_{host_port}_{host_client_port}_{sequence}"
                                        );
                                        if size == 0 {
                                ...
                                match receiver.recv().await {
                                    Ok(datagram) => match datagram.r#type.as_str() {
                                        "server_data" => {
                                            ...
                                            let protocol_str =
                                                datagram.content["protocol"].as_str();
                                            let remote_host_port_str =
                                                datagram.content["remoteHostPort"].as_str();
                                            let remote_host_client_port_str =
                                                datagram.content["remoteHostClientPort"].as_str();
                                            let server_data_settings = ServerDataSettings {
                                                ...
                                                remote_host_port: remote_host_port_str
                                                    .parse()
                                                    .unwrap(),
                                                remote_host_client_port:
                                                    remote_host_client_port_str.parse().unwrap(),
                                            };
                                            if server_data_settings.protocol == Protocol::Udp {
                                                ...
                                                match data_result {
                                                    Ok(data) => {
                                                        ...
                                                        match server_socket_sender
                                                            .send((data, dest_address))
                                                            .await
                                                        {
                                                            Ok(_) => {}
                                                            Err(error) => {
                                    ...
                                    Err(error) => {
                                        error!(
                                            "could not received data on the local client: {}",
                                            error
                                        )
                                    }
                                }
                        ...
                        let ack_handler = tokio::spawn(async move {
                            loop {
                                match ack_receiver.recv().await {
                                    ...
                                    Err(error) => {
                                        error!(
                                            "[recv]could not receive ACK for retry handling: {}",
                                            error
                                        );
                                    }
                                }
                        ...
                        tokio::spawn(async move {
                            ...
                            for _ in 0..MAX_RETRIES {
                                match tunnel_sender_clone.send(datagram.clone()) {
                                    ...
                                    Err(error) => {
                                        error!(
                                            "[send]could not send local client data to tunnel: {}",
                                            error
                                        );
                                    }
                                }
