    ...
    fn fragmentation_results_in_correct_size() {
        for i in 0..1500 {
            ...
            for fragment in &fragments[..fragments.len() - 1] {
                assert!(fragment.to_vec().unwrap().len() == max_fragment_size || fragment.to_vec().unwrap().len() == max_fragment_size-1);
            }
    ...
    fn fragmentation_data_integrity() {
        for i in 0..1500{
            ...
            data.push_str(&fragments.last().unwrap().content["data"]);
             ...
             serde_json::from_str::<ControlDatagram>(&data).map_err(|e|{
                 panic!("error for: {data}\n{e}")
             }).unwrap();
        }
    }
}
